...
function decodeBase64(value: string, expectedLength: number): Uint8Array {
  ...
  if (!/^[A-Za-z0-9+/]*={0,2}$/.test(normalized)) throw new Error("BASE64_INVALID");
...
function selfApprovalPayload(userId: string, device: DeviceRow, challengeId: string, decision: Decision): string {
  return JSON.stringify({
    protocol: "forsure-aegis-device-approval-decision",
    version: 1,
    userId,
    approverDeviceId: device.device_id,
    targetDeviceId: device.device_id,
    targetChallengeId: challengeId,
    targetDevicePublicKey: device.device_public_key,
    targetDeviceSigningKey: device.device_signing_key,
    decision,
    selfApproval: true,
  });
}

function possessionPayloadV2(challenge: ChallengeRow, device: DeviceRow): string {
  return JSON.stringify({
...
async function accountFingerprint(payload: string): Promise<string> {
...
serve(async (req) => {
  ...
  const action = typeof input.action === "string" ? input.action : "decision";
  const deviceId = typeof input.device_id === "string"
    ? input.device_id.trim()
    : typeof input.target_device_id === "string" ? input.target_device_id.trim() : "";
  ...
  if (action === "bind") {
    ...
    if (account.identity_binding_version !== 1) return respond(req, 422, { ok: false, code: "ACCOUNT_BINDING_VERSION_INVALID" });

    const bindingPayload = accountBindingPayload(account);
    if (await accountFingerprint(bindingPayload) !== account.fingerprint) {
      return respond(req, 422, { ok: false, code: "ACCOUNT_FINGERPRINT_INVALID" });
    }
    const [accountBindingValid, deviceBindingValid] = await Promise.all([
    ...
    if (!accountBindingValid) return respond(req, 422, { ok: false, code: "ACCOUNT_BINDING_SIGNATURE_INVALID" });
    if (!deviceBindingValid) return respond(req, 422, { ok: false, code: "DEVICE_AUTHORIZATION_SIGNATURE_INVALID" });

    const { data: boundData, error: boundError } = await admin.rpc("finalize_device_account_binding_v2", {
      p_user_id: user.id,
  ...
  const challengeId = typeof input.target_challenge_id === "string" ? input.target_challenge_id.trim() : "";
  const signature = typeof input.approver_signature === "string" ? input.approver_signature.trim() : "";
  ...
  if (challenge.cancelled_at || !Number.isFinite(consumedAt) || !Number.isFinite(expiresAt)
      || consumedAt > expiresAt || consumedAt + 24 * 60 * 60 * 1000 <= Date.now()) {
    return respond(req, 409, { ok: false, code: "DEVICE_ENROLLMENT_EXPIRED" });
  }
  if (challenge.possession_payload_version !== 2 || !challenge.device_possession_signature) {
    return respond(req, 422, { ok: false, code: "DEVICE_POSSESSION_V2_REQUIRED" });
  }

  const [approvalValid, possessionValid] = await Promise.all([
    verifyEd25519(device.device_signing_key, signature, selfApprovalPayload(user.id, device, challengeId, decision)),
    verifyEd25519(device.device_signing_key, challenge.device_possession_signature, possessionPayloadV2(challenge, device)),
  ]);
  if (!approvalValid) return respond(req, 422, { ok: false, code: "APPROVER_SIGNATURE_INVALID" });
  ...
  if (decision === "reject") {
    ...
    const { data: rejected, error: rejectError } = await admin
      ...
      .update({
        ...
        revoked_at: now, revoke_reason: "self_rejected_pending_device", stale_at: now,
  ...
  const { data: approvedData, error: approvedError } = await admin.rpc("finalize_self_approved_device_v2", {
    p_user_id: user.id,
