...
-- One-time, idempotent cleanup for the confirmed cross-platform DeviceID
-- collision observed on 2026-08-05. The stale routing identity is retained as
-- a revoked tombstone so an older client cannot silently claim it again.
-- Message parents, per-device ciphertext copies, inbox rows, conversations and
-- account identity keys are intentionally preserved.
...
begin
  ...
  set is_active = false,
      ...
      revoke_reason = 'cross_platform_device_id_collision',
