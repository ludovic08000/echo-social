-- Preserve account identity continuity when the active public identity row is
-- temporarily absent but an encrypted recovery vault still pins the canonical
-- fingerprint. Such an account is recoverable; it is never a new account.
...
begin
  ...
  if found then
    ...
    select vault.identity_fingerprint into v_vault_fingerprint
    from public.aegis_recovery_vaults vault
    where vault.user_id = v_uid
    limit 1
    for update;

    if found then
      if trim(coalesce(v_vault_fingerprint, '')) is distinct from trim(p_account_fingerprint) then
        return jsonb_build_object('ok', false, 'code', 'ACCOUNT_IDENTITY_MISMATCH');
      end if;
    else
      select exists (
        select 1
        from public.user_backups backup
        where backup.user_id = v_uid
      ...
      if v_backup_exists or v_other_device_exists then
        return jsonb_build_object(
          'ok', false,
          'code', 'ACCOUNT_IDENTITY_BOOTSTRAP_REQUIRES_EXPLICIT_MIGRATION'
        );
      end if;
    end if;
  end if;
...
begin
  ...
  if found then
    if v_account.identity_key is distinct from p_account_identity_key
    ...
    if not found
       or trim(coalesce(v_vault_fingerprint, '')) is distinct from trim(p_account_fingerprint) then
      return jsonb_build_object('ok', false, 'code', 'ACCOUNT_RECOVERY_IDENTITY_MISMATCH');
    end if;
  ...
  if not found then
    if v_inserted_account_id is not null then
      delete from public.user_public_keys where id = v_inserted_account_id;
    end if;
...
begin
  ...
  if exists (
    ...
    return jsonb_build_object('ok', false, 'code', 'FIRST_DEVICE_RECOVERY_VAULT_EXISTS');
  end if;

  if exists (
    select 1
    from public.user_backups backup
    where backup.user_id = p_user_id
  ) then
    return jsonb_build_object('ok', false, 'code', 'FIRST_DEVICE_BACKUP_EXISTS');
  end if;
