-- Sesame-style retry requests for multi-device message copies.
--
-- When a device cannot decrypt a message after local storage/session loss, it
-- asks the original sender to regenerate one encrypted copy for the current
-- requester device. The server never receives plaintext: it stores only the
-- request metadata and the final per-device ciphertext.
...
AS $function$
  ...
  JOIN public.conversation_participants sender_participant
   ...
   AND sender_participant.user_id = r.sender_user_id
  JOIN public.conversation_participants requester_participant
    ON requester_participant.conversation_id = m.conversation_id
   AND requester_participant.user_id = r.requester_user_id
...
GRANT EXECUTE ON FUNCTION public.list_pending_device_copy_retries(integer)
  TO authenticated;
...
BEGIN
  ...
  IF p_encrypted_body IS NULL OR length(trim(p_encrypted_body)) = 0 THEN
    RAISE EXCEPTION 'Encrypted body required';
  END IF;

  IF p_sender_device_id IS NULL OR length(trim(p_sender_device_id)) = 0 THEN
    RAISE EXCEPTION 'Sender device required';
  END IF;

  SELECT *
  INTO v_req
  FROM public.message_device_retry_requests
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Retry request not found';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    JOIN public.conversation_participants sender_participant
      ON sender_participant.conversation_id = m.conversation_id
     AND sender_participant.user_id = v_req.sender_user_id
    JOIN public.conversation_participants requester_participant
      ON requester_participant.conversation_id = m.conversation_id
     AND requester_participant.user_id = v_req.requester_user_id
    JOIN public.user_devices requester_device
      ON requester_device.user_id = v_req.requester_user_id
     AND requester_device.device_id = v_req.requester_device_id
     AND requester_device.is_active = true
    WHERE m.id = v_req.message_id
      AND m.sender_id = v_req.sender_user_id
  ) THEN
    RAISE EXCEPTION 'Retry request no longer valid';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.complete_device_copy_retry(uuid, text, text)
  TO authenticated;
