-- Déployer avec le client libsignal ; les clients aegis1 ne peuvent plus envoyer.
-- Les lignes historiques sont conservées ; seules les nouvelles écritures sont validées.
BEGIN;
ALTER TABLE public.message_device_copies DROP CONSTRAINT message_device_copies_aegis_v1_wire_check;
...
BEGIN
  -- Un seul format courant : messages Signal ou PreKey, base64 canonique non vide.
  IF p_body !~ '^aegis[.]libsignal[.][23][.][A-Za-z0-9+/]+={0,2}$' THEN RETURN false; END IF;
  encoded := split_part(p_body, '.', 4);
  RETURN replace(encode(decode(encoded, 'base64'), 'base64'), E'\n', '') = encoded;
EXCEPTION WHEN invalid_parameter_value OR data_exception THEN RETURN false;
END;
$function$;

CREATE OR REPLACE FUNCTION public.aegis_send_message(p_message_id uuid, p_conversation_id uuid, p_body text, p_image_url text, p_extra jsonb, p_copies jsonb, p_sender_device_id text, p_route_version text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  where copy.recipient_user_id is null
     ...
     or not (
       public.is_supported_aegis_device_copy(copy.encrypted_body)
     );
...
$function$
;
ALTER TABLE public.message_device_copies ADD CONSTRAINT message_device_copies_libsignal_wire_check CHECK (public.is_supported_aegis_device_copy(encrypted_body)) NOT VALID;
COMMIT;
