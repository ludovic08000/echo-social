-- Fix revoked device reactivation handling for multi-device E2EE.
--
-- The application must never silently reactivate a device that has already
-- been revoked. This migration adds safe helper RPCs that let the client
-- detect that state and register devices without bypassing the existing
-- revoked_device_cannot_be_reactivated constraint.
...
begin
  ...
  do update set
    ...
    is_active = true,
    last_seen_at = now()
  where public.user_devices.is_active = true;
