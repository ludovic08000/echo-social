...
Sesame contains adapted user-interface, retry and message-state logic from Signal Desktop:
...
- `src/lib/messaging/sesameSendState.ts`
  ...
  - Copyright 2021 Signal Messenger, LLC
  - preserves Signal's monotonic `Pending → Sent → Delivered → Read → Viewed` semantics and extends them to Sesame group-send aggregation
...
- `src/lib/messaging/signalRetryAfter.ts`
  ...
  - Copyright 2021 Signal Messenger, LLC
- `src/lib/messaging/sesameDeliveryPolicy.ts`
...
The adapted files were modified for the Sesame web messaging architecture on 2026-07-16. They replace Signal-specific Electron, Redux, localization, libsignal and component dependencies with Sesame's React, Supabase and local message-queue interfaces.
