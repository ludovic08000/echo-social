...
## Lot D — résultats (mai 2026)

### D1 — Partage d'écran ✅
- `useCall.toggleScreenShare()` + `isScreenSharing`. `setScreenShareEnabled(true,{audio:true})`. Bouton MonitorUp dans `CallOverlay` (web only). E2EE LiveKit appliqué au flux écran.

### D2 — Historique d'appels ✅
- Table `call_history` (caller/callee/conv/type/final_status/duration/started/ended). Trigger `trg_log_call_history` AFTER UPDATE sur `active_calls` quand status passe à terminal (ended/declined/cancelled/missed/no_answer/expired).
- `useCallHistory` (Realtime auto-refresh) + `CallHistoryPanel.tsx` (icônes Out/In/Missed, durée, rappel audio/vidéo en 1 tap).
- Bouton `PhoneMissed` dans header ChatWidget ouvre l'historique en overlay.

### D4 — Push réel pour appels entrants ✅
- Trigger `trg_notify_incoming_call_push` AFTER INSERT sur `active_calls` (ringing) → `pg_net.http_post` vers `/functions/v1/push-notify` (best-effort, jamais bloquant).
- `push-notify` réécrit avec **VAPID ES256 JWT** + **aes128gcm payload encryption** (RFC 8030/8188/8291). Header `Urgency: high` pour `kind=call_incoming`. Cleanup auto des subscriptions 404/410.
- **Secrets requis** : `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY` (base64url, P-256 raw), `VAPID_SUBJECT` (mailto:). Sans ces secrets, la fn renvoie `vapid_not_configured` (pas d'erreur).

### D3 — Appels de groupe ⚠️ partiel
- Colonne `active_calls.room_id` ajoutée (NULL = 1-1, dérive `call-<convId>`).
- Reste à faire : (a) UI grille N participants distants dans `CallOverlay`, (b) flow "appel de groupe" depuis conversation `is_group=true` (signal multi-callee), (c) ringer parallèle.
