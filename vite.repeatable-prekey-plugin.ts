...
function transformChatView(code: string): string {
  ...
  const requiredMarkers = [
    ...
    'decryptedCache.get(msgId, body) === text',
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (cleanId.endsWith('/src/components/messages/ChatView.tsx')) {
        const transformed = transformChatView(normalizedCode);
        return transformed === code ? null : { code: transformed, map: null };
      }
      if (cleanId.endsWith('/src/components/messages/DecryptedMessageBody.tsx')) {
        const transformed = transformDecryptedMessageBody(normalizedCode);
        return transformed === code ? null : { code: transformed, map: null };
      }
      if (cleanId.endsWith('/src/components/messages/decryptionService.ts')) {
        const transformed = transformDecryptionService(normalizedCode);
        return transformed === code ? null : { code: transformed, map: null };
      }
      return null;
    },
