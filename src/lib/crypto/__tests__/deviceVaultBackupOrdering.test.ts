...
vi.mock('../webDeviceKeyVault', () => ({ captureEncryptedWebDeviceVault: mocks.capture, restoreEncryptedWebDeviceVault: vi.fn() }));
...
beforeEach(() => {
  ...
  mocks.capture.mockResolvedValue({ version: 1, iv: 'iv', ciphertext: 'sealed' });
...
describe('cloud backup ordering', () => {
  it('captures a second snapshot only after the first upload and readback', async () => {
    ...
    mocks.capture.mockResolvedValueOnce({ version: 1, iv: 'old', ciphertext: 'old' })
      .mockResolvedValueOnce({ version: 1, iv: 'new', ciphertext: 'new' });
    ...
    expect(mocks.stored).toEqual({ version: 1, iv: 'new', ciphertext: 'new' });
  });
