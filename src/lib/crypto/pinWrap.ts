...
export async function unwrapKeysWithPin(
  ...
  const blob = await runTxOn('pin-wrap', PIN_WRAP_STORE, 'readonly', (store) =>
    reqToPromise<WrappedKeyBlob | undefined>(store.get(userId)),
  );
...
export async function hasWrappedKeys(userId: string): Promise<boolean> {
  try {
    const result = await runTxOn('pin-wrap', PIN_WRAP_STORE, 'readonly', (store) =>
      reqToPromise<WrappedKeyBlob | undefined>(store.get(userId)),
    );
    return !!result;
  } catch {
