...
export async function bindApprovedDeviceToAccountV2(
  userId: string,
  deviceId: string,
): Promise<void> {
  ...
  // This is intentionally post-PIN: prepareDeviceAuthorization uses the
  // account Ed25519 signing private key to authorize this exact device.
  ...
  const { data, error: invokeError } = await supabase.functions.invoke('approve-device-enrollment-v2', {
    body: {
      action: 'bind',
      device_id: deviceId,
