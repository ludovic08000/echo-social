...
import { Zap, ChevronRight, RefreshCw, Users } from 'lucide-react';
...
export function ZeusSection() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!user) return;
      const { data } = await supabase.from('zeus_conversations').select('id, messages').eq('user_id', user.id).order('updated_at', { ascending: false }).limit(1).maybeSingle();
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div><h2 className="text-lg font-bold text-foreground">Zeus — Assistant Stratégique Proactif</h2><p className="text-[11px] text-muted-foreground">Propose • Vous validez • Il applique</p></div>
        <Badge variant="outline" className="ml-auto text-[10px] border-amber-500/30 text-amber-400">PROACTIF</Badge>
        <Button size="sm" variant="ghost" className="text-xs text-muted-foreground h-7 gap-1" onClick={() => {
          conversationIdRef.current = null; chatHistory.current = []; isInitialLoad.current = true;
          setMessages([WELCOME_MSG]); setPendingProposals(new Map()); setAppliedProposals(new Set()); setRejectedProposals(new Set());
        }}>✨ Nouvelle</Button>
      </div>
