...
function IncomingCallHandler() {
  ...
  const call = useCall({
    ...
    onCallConnected: useCallback(() => {
      if (activeIncomingConversationIdRef.current) openChat(activeIncomingConversationIdRef.current);
    }, [openChat]),
  });
...
function MessagesWidgetRoute() {
  ...
  const { openChat } = useChatWidget();
  useEffect(() => { openChat(conversationId || undefined); }, [conversationId, openChat]);
...
function AccountKeySyncRunner() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    const controller = startRealtimeKeySync({ userId: user.id });
  ...
  return (
    <UXModeContext.Provider value={uxMode}>
      <QueryClientProvider client={queryClient}>
        <I18nProvider><AppContent /></I18nProvider>
      </QueryClientProvider>
