...
function recreateLegacyE2EEDatabase(): Promise<void> {
  return new Promise((resolve) => {
    void (async () => {
      try {
        await runTxOn('e2ee', [STORE_SESSION, STORE_PREKEYS], 'readwrite', (stores) => {
...
async function saveRatchetLocal(convId: string, state: RatchetState, x3dhHeader?: X3DHInitialMessage | null) {
  try {
    ...
    await runTxOn('ratchet', RATCHET_STORE_NAME, 'readwrite', (store) => {
      store.put(record);
    });
  } catch (e) {
...
async function loadRatchetLocal(convId: string): Promise<{ state: RatchetState; x3dhHeader: X3DHInitialMessage | null } | null> {
  try {
    const result = await runTxOn('ratchet', RATCHET_STORE_NAME, 'readonly', (store) =>
      reqToPromise<any>(store.get(convId)),
    );
...
function cleanupLegacyStorage() {
  try {
    ...
    if (!localStorage.getItem(migrationKey)) {
      ...
      void runTxOn('ratchet', RATCHET_STORE_NAME, 'readwrite', (store) => {
        store.clear();
      }).then(() => {
        console.log('[E2EE] Cleared stale ratchet states (migration v4)');
      }).catch(() => {});
      void runTxOn('e2ee', STORE_SESSION, 'readwrite', (store) => {
        store.clear();
      }).then(() => {
      ...
      try {
        ...
        if (data) {
          ...
          if (fpChanged) {
            ...
            if (conversationId) {
              void deleteRatchetLocal(conversationId);
              ratchetRef.current = null;
              peerHasRespondedRef.current = false;
              x3dhInfoRef.current = null;
              pendingPayloadRef.current.clear();
